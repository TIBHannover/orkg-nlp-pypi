...
class CSNer(ORKGNLPBaseService):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        titles_encoder: ORKGNLPBaseEncoder = CSNerEncoder(
            io.read_json(requirements["titles_alphabet"])
        )
        titles_runner: ORKGNLPBaseRunner = ORKGNLPTorchRunner(
            io.load_torch_jit(requirements["titles_model"])
        )
        titles_decoder: ORKGNLPBaseDecoder = CSNerDecoder(
            io.read_json(requirements["titles_alphabet"])
        )
        self._titles_pipeline_name = "titles"
        self._register_pipeline(
            self._titles_pipeline_name, titles_encoder, titles_runner, titles_decoder
        )

        abstracts_encoder: ORKGNLPBaseEncoder = CSNerEncoder(
            io.read_json(requirements["abstracts_alphabet"])
        )
        abstracts_runner: ORKGNLPBaseRunner = ORKGNLPTorchRunner(
            io.load_torch_jit(requirements["abstracts_model"])
        )
        abstracts_decoder: ORKGNLPBaseDecoder = CSNerDecoder(
            io.read_json(requirements["abstracts_alphabet"])
        )
        self._abstracts_pipeline_name = "abstracts"
        self._register_pipeline(
            self._abstracts_pipeline_name,
            abstracts_encoder,
            abstracts_runner,
            abstracts_decoder,
        )
    ...
    def __call__(self, title: str = None, abstract: str = None) -> Any:
        ...
        if title and abstract:
            return {
                ...
                "abstract": self._annotate(
                    q=abstract, pipeline_name=self._abstracts_pipeline_name
                ),
            }
